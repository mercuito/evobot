import { DiscordGatewayAdapterCreator, VoiceConnectionStatus, entersState, joinVoiceChannel } from "@discordjs/voice";
...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction, input: string) {
    ...
    if(!connection)
      {
      console.log(`error joining channel: ${channel.id}`)
      return
    }
    try {
      ...
      connection.receiver.speaking.on("end", (userId) => {
        const queue = bot.queues.get(interaction.guild!.id);
        if(queue && queue.isBackgrounded()){
          queue.scheduleUnbackground(connection)
        }
        else if(queue && connection.receiver.speaking.users.size == 1 && queue.backgroundScheduled && (Date.now() - queue.backgroundScheduled < 1000))
        {
          queue.unbackground()
        }
      });
